...
def _proof(name: str, service_id: bytes) -> bytes:
    scope = {
        1: name, 2: b"V" * 32, 3: None, 4: SOURCE_OPERATOR,
        5: DESTINATION_OPERATOR, 6: "eu", 7: [[8443, 8443]],
        8: [6], 9: [1], 10: False, 11: 0,
    }
    return encode_deterministic({
        1: 11, 2: 1, 32: b"proof", 33: b"A" * 32,
        34: [b"1" * 32, b"2" * 32], 35: service_id, 36: scope,
        37: SOURCE_OPERATOR, 38: DESTINATION_OPERATOR, 39: b"B" * 32,
...
def _context(name: str, service_id: bytes, proof: bytes) -> bytes:
    ...
    scope = {
        1: name, 2: service_id, 3: None, 4: SOURCE_OPERATOR,
        5: DESTINATION_OPERATOR, 6: "eu", 7: [[8443, 8443]],
        8: [6], 9: [1], 10: False, 11: 0,
    }
    return encode_deterministic({
        1: 12, 2: 1, 3: {1: 9, 2: SOURCE_OPERATOR, 3: b"auth", 4: SOURCE_OPERATOR},
        9: 1_893_455_940, 32: b"context", 33: SOURCE_OPERATOR,
        34: DESTINATION_OPERATOR, 35: service_id, 36: proof_digest,
        37: b"B" * 32, 38: b"C" * 32, 39: b"P" * 32, 40: scope,
        41: 1_893_456_300, 42: 1, 43: 7, 44: dependencies,
        45: sha256(encode_deterministic(dependencies)).digest(), 46: b"R" * 32,
    })
...
def build_authority_set(count: int) -> tuple[ServiceAuthority, ...]:
    ...
    source_authority = FederationAuthority(b"source-auth", source_context_private.public_key(), SOURCE_OPERATOR, KeyPurpose.FEDERATION_AUTHORIZATION, KeyLifecycle.ACTIVE, 1, 1, 0, 1_893_456_300, False)
    destination_authority = FederationAuthority(b"destination-auth", destination_context_private.public_key(), DESTINATION_OPERATOR, KeyPurpose.FEDERATION_AUTHORIZATION, KeyLifecycle.ACTIVE, 1, 1, 0, 1_893_456_300, False)
    ...
    for index in range(count):
        ...
        authenticated = authenticate_bilateral_context(
            sign1(context.canonical_bytes(), b"source-auth", source_context_private), source_authority,
            sign1(context.canonical_bytes(), b"destination-auth", destination_context_private), destination_authority,
            OPENED_AT,
        )
        ...
        grant = RouteGrant(
            grant_version=1, route_id=route_id, name_digest=sha256(name.encode("ascii")).digest(),
            service_id=name, source_operator_id="nbsr12df4x56n2df4x56n2df4x56n2df4x56n2df4x56n2df4x56n2dfsk5743r",
            source_edge_id="source.edge", destination_operator_id="nbsr1g3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zyg3zqel9ufg",
            destination_edge_set=("destination.edge",), allowed_transports=("tcp",), allowed_ports=(8443,),
            client_session_key_thumbprint=FIXTURES.client_session_key_thumbprint,
            not_before=1_893_455_940, expires_at=1_893_456_300,
            lease_id=_id(0x30, index), record_sequence=42, policy_hash=FIXTURES.policy_hash,
        ...
        evidence = AuthorizationEvidence(
            operator_id=SOURCE_OPERATOR, peer_operator_id=DESTINATION_OPERATOR, endpoint_operator_id=SOURCE_OPERATOR,
            service_id=service_id, authority_proof=proof, proof_pending_until=None,
            ownership_root_digest=b"A" * 32, chain_digests=frozenset({b"1" * 32, b"2" * 32}),
            canonical_name=name, revocation_commitment=b"R" * 32, trust_bundle_digest=b"B" * 32,
            trust_bundle_version=(1, 1), checkpoint_digest=b"C" * 32, checkpoint_compatible=True,
            policy_digest=b"P" * 32, allowed_protocols=frozenset({6}), allowed_ports=frozenset({8443}),
            allowed_actions=frozenset({1}), revoked_dependencies=frozenset(), now=OPENED_AT,
        )
        source_attestation, destination_attestation = attestor.authorize_and_attest(
            BilateralAuthorizer(), authenticated, evidence,
            replace(evidence, operator_id=DESTINATION_OPERATOR, peer_operator_id=SOURCE_OPERATOR, endpoint_operator_id=DESTINATION_OPERATOR),
            route_grant_digest=grant_digest, protocol=6, port=8443,
        )
        context_digest = sha256(context_wire).digest()
        transcript = [
            "NBSR-FED-ROUTE-OPEN", 2, 2, [1, 1, 1, PROFILE_ID], FIXTURES.session_id,
            request_id, channel_id, route_id, "destination.edge", FIXTURES.edge_nonce,
            "tcp", name, 8443, grant_digest, OPENED_AT, context_digest,
        ]
        route_body = {
            0: 2, 1: channel_id, 2: grant_wire, 3: FIXTURES.edge_nonce, 4: "tcp", 5: 8443,
            6: OPENED_AT, 7: session_private.sign(encode_deterministic(transcript)),
            8: {0: 1, 1: 1, 2: 1, 3: PROFILE_ID, 4: grant_digest, 5: context_digest},
        }
        result.append(ServiceAuthority(name, request_id, channel_id, route_id, grant_wire, context_wire, source_attestation, destination_attestation, encode_deterministic(route_body)))
    return tuple(result)
