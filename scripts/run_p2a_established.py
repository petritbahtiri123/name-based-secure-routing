...
def summarize_resources(samples: list[dict], completed: int, measured_seconds: float) -> dict:
    ...
    for role, values in by_role.items():
        ...
        cpu = (
            ((values[-1]["user_cpu_ns"] + values[-1]["kernel_cpu_ns"]) - (values[0]["user_cpu_ns"] + values[0]["kernel_cpu_ns"]))
            if len(values) > 1
            else 0
        )
        total_cpu += cpu
...
def run_repeat(
    ...
    try:
        ...
        record.update(
            {
                "repeat": repeat,
                "valid": validate_repeat(record),
                "operations_per_second": completed / seconds,
                "request_messages_per_second": completed / seconds,
                "response_messages_per_second": completed / seconds,
                "payload_bytes_sent": completed * payload,
                "payload_bytes_received": completed * payload,
    ...
    with tempfile.TemporaryDirectory(prefix="nbsr-p2a-") as temp_name:
        ...
        for cell in matrix:
            ...
            while repeat <= args.repeats:
                record = run_repeat(cell, repeat, binaries, authority, args.warmup_seconds, args.duration_seconds, raw)
            ...
            manifest = {
                **cell,
                ...
                "measured_seconds": args.duration_seconds,
        ...
        for key, records in grouped.items():
            ...
            def med(field: str) -> float:
                return statistics.median(r[field] for r in records if r["valid"])

            cells.append(
                {
                    "cell": key,
                    "path": records[0]["path"],
                    "streams": records[0]["streams"],
                    "payload_bytes": records[0]["payload_bytes"],
                    "valid_repeats": sum(r["valid"] for r in records),
                    "repeat_count": len(records),
                    "throughput_cv": coefficient_of_variation([r["operations_per_second"] for r in records]),
                    "median_operations_per_second": med("operations_per_second"),
                    "median_aggregate_application_gbps": med("aggregate_application_gbps"),
                    "median_p50_latency_ns": med("p50_latency_ns"),
                    "median_p95_latency_ns": med("p95_latency_ns"),
                    "median_p99_latency_ns": med("p99_latency_ns"),
                    "errors": sum(r["errors"] for r in records),
                }
            )
        analysis = {
            "schema": "nbsr-p2a-analysis-v1",
            "accepted": all(r["valid"] for r in all_records),
