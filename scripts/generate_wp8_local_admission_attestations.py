...
def admitted_attestations() -> tuple[bytes, bytes]:
    ...
    authenticated = authenticate_bilateral_context(
        sign1(context.canonical_bytes(), b"source-auth", source_private),
        source_authority,
        sign1(context.canonical_bytes(), b"destination-auth", destination_private),
        destination_authority,
        1_893_456_000,
    )
    evidence = AuthorizationEvidence(
        operator_id=b"S" * 32,
        peer_operator_id=b"D" * 32,
        endpoint_operator_id=b"S" * 32,
        service_id=SERVICE,
...
def artifacts() -> dict[str, bytes]:
    ...
    manifest = {
        ...
        "artifacts": [
            {"path": name, "length": len(wire), "sha256": hashlib.sha256(wire).hexdigest()} for name, wire in sorted(result.items())
        ],
    }
