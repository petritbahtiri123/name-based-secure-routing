...
def build(target: Path) -> dict[str, Path]:
    ...
    return {
        name: target / "release" / f"{binary}{suffix}"
        for name, binary in {"direct": "perf_direct_peer", "nbsr": "perf_rust_source", "server": "wp8_interop_server"}.items()
    }
...
def measured_resources(samples: list[dict], duration_ns: int, completed: int) -> dict:
    ...
    for role in sorted({sample["role"] for sample in measured}):
        ...
        cpu = (
            0
            if len(values) < 2
            else (values[-1]["user_cpu_ns"] + values[-1]["kernel_cpu_ns"] - values[0]["user_cpu_ns"] - values[0]["kernel_cpu_ns"])
        )
        total_cpu += cpu
        roles[role] = {
            "cpu_ns": cpu,
            "cpu_percent_assigned_mean": statistics.fmean(v["cpu_percent_assigned"] for v in values),
...
def run_shard(
    path: str, samples: int, rate: float, instrumentation: bool, binaries: dict[str, Path], authority: Path, temporary: Path, ordinal: int
) -> dict:
    ...
    try:
        ...
        if instrumentation:
            os.environ["NBSR_P2B_PROFILE"] = "1"
        else:
            os.environ.pop("NBSR_P2B_PROFILE", None)
        try:
            ...
            if previous is None:
                ...
                os.environ["NBSR_P2B_PROFILE"] = previous
        if path == "nbsr":
            ack.touch()
        ...
        def percentile(value: int) -> int:
            return latencies[max(0, (len(latencies) * value + 99) // 100 - 1)]
        ...
        return {
            ...
            "throughput": len(latencies) / (duration_ns / 1e9),
...
def aggregate(
    path: str, load: int, repeat: int, instrumentation: bool, duration: int, binaries: dict[str, Path], authority: Path, temporary: Path
) -> dict:
    rate = LOADS[path][load]
    counts = [round(rate * duration)] if path == "direct" else shard_plan(rate, duration)
    shards = [
        run_shard(path, count, rate, instrumentation, binaries, authority, temporary, ordinal) for ordinal, count in enumerate(counts, 1)
    ]
    ...
    return {
        "schema": "nbsr-p2b-logical-profile-v1",
        "path": path,
        "load_percent": load,
        "repeat": repeat,
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="nbsr-p2b-") as temporary_name:
        ...
        if args.suite == "smoke":
            jobs = [(path, 50, 1, True, 5) for path in ("direct", "nbsr")]
        elif args.suite == "observer":
            jobs = [(path, 50, repeat, enabled, 30) for path in ("direct", "nbsr") for repeat in range(1, 4) for enabled in (False, True)]
        else:
            jobs = [(path, load, repeat, True, 180) for path in ("direct", "nbsr") for load in (50, 90) for repeat in range(1, 4)]
