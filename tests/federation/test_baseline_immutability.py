...
from nbsr.federation.profile import CoreBaselineError, assert_core_baseline_lock_authority, assert_f75_core_overlay
...
def _copy_locked_baseline(destination: Path) -> None:
    ...
    target_overlay = destination / OVERLAY_PATH.relative_to(ROOT)
    target_overlay.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(OVERLAY_PATH, target_overlay)
...
def test_core_v02_baseline_accepts_exact_110_artifact_inventory() -> None:
    assert assert_core_baseline_lock_authority(ROOT) is None
