...
**B4b closure:** `scripts/run_b4b_mixed_connections.py` combines an established 1 KiB / 8-stream forwarding pair with independent client/destination process pairs. Every admission client creates four fresh QUIC transport connections and, on each connection, a ControlSession, federated route/channel, and application stream. The authoritative three-repeat sweep covered 0/1/2/4/8/16/32/64/128 clients. It was STABLE through 8 clients, DEGRADED from 16 through 64, and SATURATED at 128. At 128 clients the median result was 168/512 successful admissions, 22.22 admissions/s, 0.271 Gbit/s established forwarding (0.699345 of baseline), 1.092 ms established p99, and 1,083,265,024 bytes peak working set; failures were preserved and owned processes/resources cleaned up. Status: Evidence PASS / System SATURATED. This is Windows loopback host-contention evidence with separate destination runtimes, not shared-runtime, WAN, server-class, production, or hardware-limit evidence. Raw and derived evidence: `evidence/performance/mixed-connections-b4b-d45fc06-r5/`.
