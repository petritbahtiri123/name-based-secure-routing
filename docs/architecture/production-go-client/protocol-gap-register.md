...
|---|---|---|---|---|
| Live RouteGrant acquisition | Core verifies/uses grants; Go peer loads fixtures; no live acquisition exchange | Production channel creation and TS-B authorization | **REQUIRES_PROTOCOL_DECISION** | Approve one authenticated local Authority Provider API first; if impossible, separately approve a protocol exchange |
...
1. Approve the **local Authority Provider** direction and freeze its
   authentication, request/response, idempotency, renewal, cancellation,
   freshness, and revocation-feed contract.
...
5. Confirm **user-space proxy first** as the first adapter, with TUN and Windows
   native interception deferred to separately accepted platform tranches.
