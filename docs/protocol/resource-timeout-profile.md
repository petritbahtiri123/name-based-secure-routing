...
| WP4 same-edge resume authority | **Lab profile:** earliest of 30 seconds, remaining grant validity, any shorter authority, and the old-session 60-minute hard deadline | Sealed absolute process-monotonic domain; each session stores absolute creation and hard-deadline instants; public resume APIs accept no time input; deadline equality is expired |
...
| WP4 mandatory audit queue | **Lab profile:** 1,024 events in one queue shared across channels within a Transport Session | Reject audited mutation before state change at capacity |
