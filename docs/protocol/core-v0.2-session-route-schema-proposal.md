...
The lab's bounded AuditLog retains exactly 1024 queued events in one global
queue, with no smaller hidden per-channel cap or reserved partition. Every
mandatory audited mutation reserves one of those exact 1024 slots before
changing route, binding, stream, UDP, quota, lifecycle, or resume state.
Exhaustion rejects the affected operation before mutation; it does not mutate,
close, revoke, or otherwise disturb sibling channels. Popping any event releases
one global slot.
