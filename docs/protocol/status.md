...
verification and public-safe packet evidence pass. A genuinely independent
Go/quic-go source exchanges the frozen Core v0.2 and F75 route/stream path with
the Rust/Quinn destination over ALPN `nbsr-quic-1`; independent route/stream
wire interoperability is `PROVEN`. The final conformance matrix passed with
2,594 passed, 0 failed, and one documented platform skip; both independent
reviews returned `READY`. WP8 evidence closure is complete and ready for human
approval. The
...
**WP8 threshold-container supplement:** `WP8-THRESHOLD-CONTAINER-01` is
...
This is payload and semantic validation evidence only: the proposed
registrar/witness and recovery threshold container is not approved or consumed,
and no Task 5
...
transport packaging is defined only by the unapproved threshold-container
supplement: a single trust
...
| Tranche 2B ACP authority wire (Acquire/Renew/Freshness, HTTP/2) | FROZEN — Approved for implementation | `/acp/authority`, mandatory TLS 1.3, deterministic CBOR/COSE envelopes, and dedicated result signer purpose. |
| Tranche 2B Enrollment Profile v1 | FROZEN — Approved for implementation | `/acp/enroll`, initial enrollment-only profile, mandatory TLS 1.3/HTTP/2 transport, and dedicated `ENROLLMENT_RESULT_SIGNING` result signing purpose. Reenrollment/rotation/recovery/replacement remain future protocol work and are not silently supported. |
...
| Public conformance suite and Internet-Draft | Implemented | Task 10B adds a reproducible independent Go/quic-go source to Rust/Quinn destination route/stream exchange. The final matrix and two independent reviews pass. The draft is not an approved standard. |
