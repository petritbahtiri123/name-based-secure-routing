...
| Reusable multi-service Transport Session with isolated Service Channels | Planned | WP4 implementation is approved and in progress at the origin-free lab boundary; session reuse, isolated channels, exporter binding, bounded lifecycle, same-edge resume, and native QUIC DATAGRAM remain unimplemented runtime work. No OriginSet selection, Origin Endpoint connection, NameRelay integration, production claim, cross-edge resume, 0-RTT, or frozen Core v0.1 change is authorized. |
